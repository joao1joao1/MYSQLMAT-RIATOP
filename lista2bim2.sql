...
select titulo, editora from Livros where editora = 'HarperCollins';

select titulo, ano_publicacao from Livros where ano_publicacao between 2000 and 2010;

select autor, count(titulo) as 'qtd_livros' from Livros
group by autor
having qtd_livros > 3;

select titulo from Livros where titulo like '%Senhor%';

select titulo, categoria from Livros where categoria in ('Fantasia', 'Ficção Científica', 'Fábula');

select distinct idioma from Livros;

select titulo, quantidade_paginas from Livros where quantidade_paginas between 200 and 400;
